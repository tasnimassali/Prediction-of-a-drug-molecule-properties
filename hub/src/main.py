...
import hub.src.feature_extractor as feature_extractor
...
def generate_tokens(smiles, len_percentile=100):
    ...
    # print('True max length is ' + str(np.max(smiles_len)) + ', ' + str(max_phrase_len) + ' is set the length cutomodel1.')
...
def get_prediction_score(y_label, y_predict):
    """Evaluate predictions using dimodel1erent evaluation metrics.
...
def create_model(model_type, num_words, input_length, output_dim=1, dropout_rate=0.0):
    ...
    model = Sequential()
    if model_type == 'model1': # Feed Forward Neural Network
...
def Train(model_type = config.MODEL_TYPE):
    ...
    checkpointer = ModelCheckpoint(os.path.join(models_folder, model_type + '.keras'), monitor='val_acc', verbose=1, save_weights_only=False, save_best_only=False, mode='auto') 
...
def Predict(X_test, model_type = config.MODEL_TYPE):
    ...
    """
    models_folder = 'hub/models'
    path = os.path.join(models_folder, model_type + '.keras')
...
def run():
    ...
    # prediction
    y_pred = Predict(X_test, model_type = config.MODEL_TYPE)

    # evaluation
    model_eval = Evaluate(y_test, y_pred)
    print(model_eval)
